...
public class InMemoryRepositoryStrategy<TItem>
    ...
    public override Chunk<TItem> GetBlock(Expression<Func<TItem, bool>> isNotStart, uint blockSize = DefaultBlockSize) {
        var items = _repository.Query.SkipWhile(isNotStart)
                   .Take((int)blockSize)
                   .ToArray();
        return new() {
            Size = blockSize,
    ...
    public override TItem? Find(Expression<Func<TItem, bool>> predicate)
        => _repository.Query.FirstOrDefault(predicate);
    ...
    public override void Patch(Expression<Func<TItem, bool>> predicate, Action<TItem> setItem) {
        var itemToPatch = _repository.Query.FirstOrDefault(predicate);
    ...
    private bool TryRemove(Expression<Func<TItem, bool>> predicate) {
        var itemToRemove = _repository.Query.FirstOrDefault(predicate);
        if (itemToRemove is null)
            return false;
        _repository.Data.Remove(itemToRemove);
        return true;
    }
    ...
    public override ValueTask<TItem[]> GetAllAsync(CancellationToken ct = default)
        => _repository.AsyncQuery.ToArrayAsync(ct);
    ...
    public override async ValueTask<Page<TItem>> GetPageAsync(uint pageIndex = 0, uint pageSize = DefaultPageSize, CancellationToken ct = default) {
        var count = await _repository.AsyncQuery.CountAsync(ct);
        var items = await _repository.AsyncQuery.Skip((int)(pageIndex * pageSize))
                   .Take((int)pageSize)
                   .ToArrayAsync(ct);
        return new() {
            TotalCount = (uint)count,
    ...
    public override async ValueTask<Chunk<TItem>> GetBlockAsync(Expression<Func<TItem, bool>> isNotStart, uint blockSize = DefaultBlockSize, CancellationToken ct = default) {
        var items = await _repository.AsyncQuery.SkipWhile(isNotStart)
                   .Take((int)blockSize)
                   .ToArrayAsync(ct);
        return new() {
            Size = blockSize,
    ...
    public override ValueTask<TItem?> FindAsync(Expression<Func<TItem, bool>> predicate, CancellationToken ct = default)
        => _repository.Query.FirstOrDefaultAsync(predicate, ct);
    ...
    public override async Task PatchAsync(Expression<Func<TItem, bool>> predicate, Func<TItem, CancellationToken, Task> setItem, CancellationToken ct = default) {
        var itemToPatch = await _repository.AsyncQuery.FirstOrDefaultAsync(predicate, ct);
        if (itemToPatch is null)
            return;
    ...
    private async Task<bool> TryRemoveAsync(Expression<Func<TItem, bool>> predicate, CancellationToken ct = default) {
        var itemToRemove = await _repository.AsyncQuery.FirstOrDefaultAsync(predicate, ct);
        if (itemToRemove is null)
            return false;
        _repository.Data.Remove(itemToRemove);
        return true;
