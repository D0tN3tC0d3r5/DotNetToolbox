...
public partial class InMemoryDataSourceTests {
    ...
    public void Find_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.Find(_ => true);
        action.Should().Throw<NotImplementedException>();
    }

    [Fact]
    public void FindByKey_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.FindByKey(0);
        action.Should().Throw<NotImplementedException>();
    }

    [Fact]
    public async Task FindAsync_BaseStrategy_ShouldThrow() {
        var action = async () => await _dummyDataSource.FindAsync(_ => true);
        await action.Should().ThrowAsync<NotImplementedException>();
    }
    [Fact]
    public async Task FindByKeyAsync_BaseStrategy_ShouldThrow() {
        var action = async () => await _dummyDataSource.FindByKeyAsync(0);
        await action.Should().ThrowAsync<NotImplementedException>();
    }
