...
public partial class InMemoryDataSourceTests {
    ...
    public void Patch_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.Patch(_ => true, _ => { });
        action.Should().Throw<NotImplementedException>();
    }

    [Fact]
    public async Task PatchAsync_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.PatchAsync(_ => true, (_, _) => Task.CompletedTask);
        await action.Should().ThrowAsync<NotImplementedException>();
    }

    [Fact]
    public void Patch_WithKey_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.Patch(0, _ => { });
        action.Should().Throw<NotImplementedException>();
    }

    [Fact]
    public async Task PatchAsync_WithKey_BaseStrategy_ShouldThrow() {
        var action = () => _dummyDataSource.PatchAsync(0, (_, _) => Task.CompletedTask);
        await action.Should().ThrowAsync<NotImplementedException>();
    }
