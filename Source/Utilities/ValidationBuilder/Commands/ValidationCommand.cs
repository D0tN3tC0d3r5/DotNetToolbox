...
public abstract class ValidationCommand(string source) : IValidationCommand {
    ...
    public virtual Result Validate(object? subject)
        => subject is null || ValidateAs(subject)
            ? Result.Success()
            : Result.Invalid(Source, string.Format(null, ValidationErrorMessage, GetErrorMessageArguments(subject)));

    public virtual Result Negate(object? subject)
        => subject is null || !ValidateAs(subject)
            ? Result.Success()
            : Result.Invalid(Source, string.Format(null, InvertMessage(ValidationErrorMessage), GetErrorMessageArguments(subject)));

    protected Func<object, bool> ValidateAs { get; init; } = _ => true;
    protected CompositeFormat ValidationErrorMessage { get; init; } = MustBeValid;
