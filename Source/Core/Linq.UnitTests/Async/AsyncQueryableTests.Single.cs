...
public partial class AsyncQueryableTests {
    ...
    public void SingleOrDefault_WithInteger_ReturnsElement() {
        ...
        var result = _repo.SingleOrDefault(static x => x.Name == "BB");
    ...
    public void Single_WithInteger_ReturnsElement() {
        var expectedItem = new TestEntity("BB");
        var result = _repo.Single(static x => x.Name == "BB");
        result.Should().Be(expectedItem);
    }
    ...
    public async Task SingleAsync_ForEmptySet_Throws() {
        var result = static async () => await _emptyRepo.SingleAsync();
    ...
    public async Task SingleAsync_WithValidPredicate_ReturnsElement() {
        var expectedItem = new TestEntity("BB");
        var result = await _repo.SingleAsync(static x => x.Name == "BB");
        result.Should().Be(expectedItem);
    }

    [Fact]
    public async Task SingleAsync_WithDuplicatedItem_Throws() {
        var result = static async () => await _repoWithDuplicate.SingleAsync(static x => x.Name == "CCC");
        await result.Should().ThrowAsync<InvalidOperationException>();
    }

    [Fact]
    public async Task SingleAsync_WithInvalidPredicate_ReturnsElement() {
        var result = static async () => await _emptyRepo.SingleAsync(static x => x.Name == "K");
        await result.Should().ThrowAsync<InvalidOperationException>();
    }
}
