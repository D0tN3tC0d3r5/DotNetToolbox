...
public partial class AsyncQueryableTests {
    ...
    public void ToHashSet_WithMapping_ReturnsMappedHashSet() {
        ...
        var result = _repo.ToHashSet(static x => $"{x.Name}*");
        result.Should().BeEquivalentTo(expectedHashSet);
    }

    [Fact]
    public void ToHashSet_WithComparer_ReturnsHashSet() {
        var expectedHashSet = new[] { "A*", "BB*", "CCC*" };
        var result = _repo.ToHashSet(static x => $"{x.Name}*", EqualityComparer<string>.Default);
        result.Should().BeEquivalentTo(expectedHashSet);
    }

    [Fact]
    public void ToHashSet_WithMappingAndComparer_ReturnsMappedHashSet() {
        var expectedHashSet = new[] { "A*", "BB*", "CCC*" };
        var result = _repo.ToHashSet(static x => $"{x.Name}*");
        result.Should().BeEquivalentTo(expectedHashSet);
    }
    ...
    public async Task ToHashSetAsync_WithMapping_ReturnsMappedHashSet() {
        var expectedHashSet = new[] { "A*", "BB*", "CCC*" };
        var result = await _repo.ToHashSetAsync(static x => $"{x.Name}*");
        result.Should().BeEquivalentTo(expectedHashSet);
    }
    ...
    public async Task ToHashSetAsync_WithMappingAndIndex_ReturnsMappedHashSet() {
        ...
        var result = await _repo.ToHashSetAsync(static (x, i) => $"{i}:{x.Name}");
        result.Should().BeEquivalentTo(expectedHashSet);
    }

    [Fact]
    public async Task ToHashSetAsync_ForBigRepo_ReturnsHashSet() {
        var expectedHashSet = Enumerable.Range(0, 1000).ToHashSet(static x => new TestEntity($"{x}"));
    ...
    public async Task ToHashSetAsync_ForDuplicatedValues_ReturnsHashSet() {
        var subject = new AsyncQueryable<TestEntity>([new("A"), new("A"), new("BB"), new("BB"), new("BB"), new("CCC")]);
        var expectedHashSet = new TestEntity[] { new("A"), new("BB"), new("CCC") };
    ...
    public async Task ToHashSetAsync_WithComparer_WithMapping_ReturnsMappedHashSet() {
        var expectedHashSet = new[] { "A*", "BB*", "CCC*" };
        var result = await _repo.ToHashSetAsync(static x => $"{x.Name}*", EqualityComparer<string>.Default);
        result.Should().BeEquivalentTo(expectedHashSet);
    }
    ...
    public async Task ToHashSetAsync_WithComparer_WithMappingAndIndex_ReturnsMappedHashSet() {
        var expectedHashSet = new[] { "0:A", "1:BB", "2:CCC" };
        var result = await _repo.ToHashSetAsync(static (x, i) => $"{i}:{x.Name}", EqualityComparer<string>.Default);
        result.Should().BeEquivalentTo(expectedHashSet);
    }
