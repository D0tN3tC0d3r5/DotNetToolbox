...
public static partial class AsyncQueryableExtensions {
    ...
    public static ValueTask<TResult[]> ToArrayAsync<TItem, TResult>(this IAsyncQueryable<TItem> source, Expression<Func<TItem, TResult>> mapping, CancellationToken ct = default)
        => IsNotNull(source).Select(mapping).MakeArray(ct);

    public static ValueTask<TResult[]> ToArrayAsync<TItem, TResult>(this IAsyncQueryable<TItem> source, Expression<Func<TItem, int, TResult>> mapping, CancellationToken ct = default)
        => IsNotNull(source).Select(mapping).MakeArray(ct);
