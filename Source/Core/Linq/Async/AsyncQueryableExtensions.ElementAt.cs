...
public static partial class AsyncQueryableExtensions {
    public static async ValueTask<TItem?> ElementAtAsync<TItem>(this IAsyncQueryable<TItem> source, int index, CancellationToken ct = default) {
        IsNotNegative(index);
        var count = 0;
    ...
    public static async ValueTask<TItem?> ElementAtAsync<TItem>(this IAsyncQueryable<TItem> source, Index index, CancellationToken ct = default) {
        if (!index.IsFromEnd)
