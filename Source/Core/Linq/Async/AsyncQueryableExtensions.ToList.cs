...
public static partial class AsyncQueryableExtensions {
    ...
    public static ValueTask<List<TResult>> ToListAsync<TItem, TResult>(this IAsyncQueryable<TItem> source, Expression<Func<TItem, TResult>> mapping, CancellationToken ct = default)
        => IsNotNull(source).Select(mapping).MakeListAsync(ct);

    public static ValueTask<List<TResult>> ToListAsync<TItem, TResult>(this IAsyncQueryable<TItem> source, Expression<Func<TItem, int, TResult>> mapping, CancellationToken ct = default)
        => IsNotNull(source).Select(mapping).MakeListAsync(ct);
