...
public class GuidProvider {
    ...
    public virtual Guid ParseExact(string input, [StringSyntax(GuidFormat)] string format)
        => Guid.ParseExact(input, format);
    public virtual Guid ParseExact(ReadOnlySpan<char> input, [StringSyntax(GuidFormat)] ReadOnlySpan<char> format)
        => Guid.ParseExact(input, format);
    ...
    public virtual bool TryParseExact([NotNullWhen(true)] string? input, [NotNullWhen(true), StringSyntax(GuidFormat)] string? format, out Guid result)
        => Guid.TryParseExact(input, format, out result);
    public virtual bool TryParseExact(ReadOnlySpan<char> input, [StringSyntax(GuidFormat)] ReadOnlySpan<char> format, out Guid result)
        => Guid.TryParseExact(input, format, out result);
}
